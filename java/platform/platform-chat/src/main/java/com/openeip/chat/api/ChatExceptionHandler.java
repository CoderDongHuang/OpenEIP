...
@RestControllerAdvice(basePackages = "com.openeip.chat")
public class ChatExceptionHandler {
  ...
  public ResponseEntity<ApiEnvelope<Void>> handle(
      ChatException exception, HttpServletRequest request) {
    return error(
  ...
  public ResponseEntity<ApiEnvelope<Void>> invalid(
    ...
    return error(400, "CHAT-V-001", "Invalid Chat request", request);
  }
  ...
  public ResponseEntity<ApiEnvelope<Void>> unexpected(
      Exception exception, HttpServletRequest request) {
    LOGGER.error(
        "Unhandled Chat error requestId={} type={}",
        RequestIdFilter.get(request),
        exception.getClass().getName());
    return error(500, "CHAT-S-002", "Internal server error", request);
  }
